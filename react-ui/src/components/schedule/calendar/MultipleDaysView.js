...
const renderDaysHeader = (dates) => {
  return dates.map((date, i) => (
    <DayHeader key={i} style={styles.dayHeader(dates.length, i)} date={date} />
  ));
};

const renderDays = (dates, children) => {
  return dates.map((date, i) => (
    <div key={i} style={styles.dayEvent(dates.length, i)}>
      {renderDayEvents({
        events: children,
        date: date,
      })}
...
export default class MultipleDaysView extends PureComponent {
  ...
  render() {
    const { dates, isScrollDisable, children } = this.props;

    return (
      <div style={styles.container}>
        <div style={styles.firstColumn} />
        <div style={styles.headerContainer}>{renderDaysHeader(dates)}</div>
        <div
        ...
        >
          ...
          <div style={styles.daysContainer}>{renderDays(dates, children)}</div>
        </div>
      </div>
    );
  }
