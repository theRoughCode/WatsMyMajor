...
import { skyBlue, red, lightGreen2, blueGreenHighlight, mustard } from 'constants/Colours';
...
export default class MyCourseAppBar extends Component {
  ...
  render() {
    const { showTrashOnDrag, isDraggingCourse, showClearButton, isEditing } = this.props;

    const addTermDialogActions = [
      <FlatButton label="Cancel" primary onClick={this.closeAddTermDialog} />,
      <FlatButton label="Submit" primary onClick={this.onAddTerm} />,
    ];
    ...
    const clearDialogActions = [
      <FlatButton label="Cancel" primary onClick={this.closeClearDialog} />,
      <FlatButton label="Confirm" primary onClick={this.onClear} />,
    ];
        ...
        <div style={styles.innerContainer(showTrash)}>
          <div style={styles.titleContainer}>
            <MediaQuery minWidth={552}>
              {(matches) => <span style={styles.title(matches)}>My Courses</span>}
            </MediaQuery>
          </div>
          <div style={styles.buttonContainer}>
            ...
            <MediaQuery maxWidth={821}>
              ...
              >
                {isEditing && <MenuItem onClick={this.openAddTermDialog}>Add Term</MenuItem>}
                <MenuItem onClick={this.openImportDialog}>Import Courses</MenuItem>
                {isEditing && showClearButton && (
                  <MenuItem onClick={this.openClearDialog}>Clear Board</MenuItem>
                )}
                {isEditing ? (
                  <MenuItem onClick={this.onEditOff}>Save Changes</MenuItem>
                ) : (
                  <MenuItem onClick={this.onEditOn}>Edit Board</MenuItem>
                )}
                {isEditing && <MenuItem onClick={this.onCancel}>Discard Changes</MenuItem>}
              </Menu>
            </MediaQuery>
          </div>
          <div>
