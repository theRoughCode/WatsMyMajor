...
const getSubjectCourses = (subject, excluding, myCourses) => {
  ...
  if (coreSubjects.hasOwnProperty(subject)) {
    ...
    return subjects
      .map((s) =>
        myCourses.hasOwnProperty(s) ? { subject: s, catNums: Object.keys(myCourses[s]) } : null
      )
      .filter((c) => c != null);
  }
...
export default class LevelCheck extends Component {
  ...
  markTaken = (courses) =>
    courses.forEach(
      ({ subject, catalogNumber }) => (this.props.myCourses[subject][catalogNumber] = true)
    );
  ...
  render() {
    ...
    const subjectName = coreSubjects.hasOwnProperty(subject) ? coreSubjects[subject].name : subject;
    return (
