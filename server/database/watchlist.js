...
function addWatcher(term, classNum, username) {
  return watchlistRef.child(`${term}/${classNum}/watchers/${username}`).set(true);
}

// Removes a watcher from a class
function removeWatcher(term, classNum, username) {
  return watchlistRef.child(`${term}/${classNum}/watchers/${username}`).remove();
}

// Sets last updated enrollment numbers
function setEnrollment(term, classNum, info) {
  return watchlistRef.child(`${term}/${classNum}/enrollment`).set(info);
}

// Sets subject
function setSubject(term, classNum, subject) {
  return watchlistRef.child(`${term}/${classNum}/subject`).set(subject.toUpperCase());
}

// Sets catalog number
function setCatalogNumber(term, classNum, catalogNumber) {
  return watchlistRef.child(`${term}/${classNum}/catalogNumber`).set(catalogNumber);
}
...
async function getSubjectAndCatNum(term, classNum) {
  try {
    ...
    const catalogNumberSnapshot = await watchlistRef
