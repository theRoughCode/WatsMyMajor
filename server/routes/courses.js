...
CoursesRouter.get('/title/:subject/:catalogNumber', async function (req, res) {
  ...
  const { err, title } = await courseListDB.getCourseTitle(
    subject,
    catalogNumber
  );
  if (err) res.status(400).send(err);
...
CoursesRouter.get('/info/:subject/:catalogNumber', async function (req, res) {
  ...
  let { err, info } = await courses.getCourseInfo(
    subject.toUpperCase(),
    catalogNumber
  );
...
CoursesRouter.get('/classes/:term/:subject/:catalogNumber', async function (
  req,
  res
) {
  const { subject, catalogNumber, term } = req.params;
  const { err, classes } = await courseClassesDB.getClasses(
    subject.toUpperCase(),
    catalogNumber,
    term
  );
...
CoursesRouter.get('/reviews/:subject/:catalogNumber', async function (
  req,
  res
) {
  const { subject, catalogNumber } = req.params;
  const { err, reviews } = await reviewsCore.getCourseReviews(
    subject.toUpperCase(),
    catalogNumber
  );
  if (err) res.status(400).send(err);
...
// Add course review
CoursesRouter.post(
  '/reviews/:subject/:catalogNumber/add',
  ...
  async function (req, res) {
    ...
    try {
      let err = await reviewsCore.addCourseReview(
        subject.toUpperCase(),
        catalogNumber,
        username,
        review
...
CoursesRouter.delete(
  '/reviews/:subject/:catalogNumber/remove',
  passport.authenticate('jwt', { session: false }),
  async function (req, res) {
    const username = req.user;
    const { subject, catalogNumber } = req.params;
    ...
    try {
      let err = await reviewsCore.deleteCourseReview(
        subject.toUpperCase(),
        catalogNumber,
        username
      );
...
CoursesRouter.post(
  '/reviews/:subject/:catalogNumber/vote',
  passport.authenticate('jwt', { session: false }),
  async function (req, res) {
    const username = req.user;
    const { subject, catalogNumber } = req.params;
    ...
    try {
      let err = await reviewsCore.addCourseReviewVote(
        subject.toUpperCase(),
        catalogNumber,
        id,
        username,
